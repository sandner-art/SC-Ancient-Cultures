...
% \usepackage{natbib}    % Or your preferred citation package
...
% The \resizebox command scales the table to the width of the text.
% The '!' for height maintains the aspect ratio.
\resizebox{\textwidth}{!}{%
\begin{tabular}{l l S[table-format=3.1] c l}
\toprule
\textbf{Derivation Method} & \textbf{Principle} & {\textbf{Length (mm)}} & \textbf{Length in 1/16 \textit{djeba}} & \textbf{Supporting Evidence} \\
\midrule
\textbf{Ideal Geometric} & Modern $\pi / 6$ & 523.6 & \textbf{448.0} (by definition) & Glen Dash Pyramid Survey; Meter Congruences \\
\addlinespace
\textbf{Rhind Papyrus $\pi$} & $(16/9)^2 / 6$ & 526.7 & $\approx450.6$ & Rhind \& Moscow Papyri (Known Egyptian Math) \\
\addlinespace
\textbf{Architectural $\pi$} & $(22/7) / 6$ & 523.8 & $\approx448.2$ & Great Pyramid P/H Ratio; Maya's Cubit Rod \\
\addlinespace
\textbf{Construction} & $28 \times \text{\textit{djeba}}$ & {523.6 -- 529.2} & \textbf{448 units} (by definition) & Full Range of Archaeological Cubit Rods (Lepsius) \\
\bottomrule
\end{tabular}%
} % End of \resizebox
\end{table}
...
\textbf{Royal Cubit Dodecagon (Figure~\ref{fig:dodecagon_harmonies}).} This dodecagon is constructed with a side length ($s_{12}$) of one royal cubit ($\pi/6\,\text{m}$). The construction provides three distinct layers of the $\pi/6$ paradigm:
    \par\medskip % Adds a small vertical space for readability
